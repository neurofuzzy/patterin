...
export const API_DATA: Record<string, TypeInfo> = {
    ...
    'ShapeContext': {
        ...
        methods: {
            ...
            'translate': { params: ['x: number', 'y: number'], returns: 'this', doc: 'Translate by delta' },
            'offset': { params: ['distance: number', 'count?: number', 'miterLimit?: number', 'includeOriginal?: boolean'], returns: 'ShapeContext | ShapesContext', doc: 'Inset/outset shape outline (count>0: returns offset copies only by default)' },
            'expand': { params: ['distance: number', 'count?: number', 'miterLimit?: number', 'includeOriginal?: boolean'], returns: 'ShapeContext | ShapesContext', doc: 'Expand/outset shape' },
            'inset': { params: ['distance: number', 'count?: number'], returns: 'ShapeContext | ShapesContext', doc: 'Inset shape' },
            'subtract': { params: ['other: Shape | ShapeContext | ShapesContext'], returns: 'ShapesContext', doc: 'Subtract another shape from this one' },
    ...
    'ShapesContext': {
        ...
        methods: {
            ...
            'inset': { params: ['distance: number', 'count?: number'], returns: 'ShapesContext', doc: 'Inset shapes (alias involved offset)' },
            'subtract': { params: ['other: Shape | ShapeContext | ShapesContext'], returns: 'ShapesContext', doc: 'Subtract another shape from each shape in collection' },
            'compound': { params: [], returns: 'this', doc: 'Treat these shapes as a single compound path (rendering only)' },
            'moveTo': { params: ['x: number', 'y: number'], returns: 'this', doc: 'Move collective center to position' },
            'moveTo': { params: ['x: number', 'y: number'], returns: 'this', doc: 'Move collective center to position' },
            'x': { params: ['xPos: number'], returns: 'this', doc: 'Set x position of collective center' },
            'y': { params: ['yPos: number'], returns: 'this', doc: 'Set y position of collective center' },
        ...
        getters: {
            ...
            'vertices': { returns: 'Vertex[]', doc: 'All vertices from all shapes (flattened)' },
            'segments': { returns: 'Segment[]', doc: 'All segments from all shapes (flattened)' },
            'points': { returns: 'PointsContext', doc: 'All vertices as PointsContext' },
            'lines': { returns: 'LinesContext', doc: 'All segments as LinesContext' },
            'center': { returns: 'Vector2', doc: 'Center of all shapes' },
        },
    },
