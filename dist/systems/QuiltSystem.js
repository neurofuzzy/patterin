...
export class QuiltSystem extends BaseSystem {
    ...
    _createFlyingGeese(x, y, size, rotation) {
        const shapes = [];
        const half = size / 2;
        const tl = new Vector2(x, y);
        const tr = new Vector2(x + size, y);
    ...
    // =========================================================================
    getNodes() {
        return this._quiltPlacements.map(p => new Vertex(p.x + this._blockSize / 2, p.y + this._blockSize / 2));
    }
    filterByMask(shape) {
    ...
    _getCenter() {
        ...
        return new Vector2(bounds.minX + (bounds.maxX - bounds.minX) / 2, bounds.minY + (bounds.maxY - bounds.minY) / 2);
    }
}
