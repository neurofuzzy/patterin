 ...
 */
export class Shape {
    constructor(segments, winding = 'ccw') {
     ...
     */
    removeDegenerate(epsilon = 1e-10) {
    ...
    containsPointInternal(point, epsilon) {
        const verts = this.vertices;
        let inside = false;
     ...
     */
    static fromPoints(points, winding = 'ccw') {
        if (points.length < 3) {
     ...
     */
    static regularPolygon(n, radius, center = Vector2.zero(), rotationOffset = 0) {
        if (n < 3) {
