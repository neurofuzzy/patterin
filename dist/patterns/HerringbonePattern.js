 ...
 * ```
 */
export function createHerringbone(options) {
    ...
    for (let row = 0; row < numRows; row++) {
        ...
        for (let col = 0; col < numCols; col++) {
            ...
            const brick1 = createRotatedRect(brick1Center, brickWidth, brickHeight, angle);
            brick1.group = 'angle1';
            shapes.push(brick1);
            // Second orientation (descending from left to right) - forms V with first
            const brick2Center = new Vector2(baseX + diagWidth / 2, baseY + diagHeight);
            const brick2 = createRotatedRect(brick2Center, brickWidth, brickHeight, -angle);
            brick2.group = 'angle2';
            shapes.push(brick2);
        }
    }
 ...
 */
function createRotatedRect(center, width, height, angleDeg) {
